...
df['Lugar'] = df['Nombre departamento']
...
df['Lugar'].replace(
    to_replace=['BARRANQUILLA'],
    value='ATLANTICO',
    inplace=True
)

df['Lugar'].replace(
    to_replace=['ATLANTICO'],
    value='ATLANTICO',
    inplace=True
)
...
Atención = df.pivot_table(index='Ubicación del caso', values='Numero',aggfunc='count').reset_index()

Atención_casa = Atención[Atención['Ubicación del caso'] == 'Casa']
Atención_Fallecido = Atención[Atención['Ubicación del caso'] == 'Fallecido']
Atención_Hospital = Atención[Atención['Ubicación del caso'] == 'Hospital']
Atención_Hospital_UCI = Atención[Atención['Ubicación del caso'] == 'Hospital UCI']
Atención_Recuperado = Atención[Atención['Ubicación del caso'] == 'Recuperado']


fig = go.Figure()
fig.add_trace(go.Bar(x=Atención_casa['Ubicación del caso'],
                y=Atención_casa['Numero'],
                name='Casa',
                marker_color='rgb(55, 83, 109)'
                ))
fig.add_trace(go.Bar(x=Atención_Fallecido['Ubicación del caso'],
                y=Atención_Fallecido['Numero'],
                name='Fallecidos',
                marker_color='rgb(26, 118, 255)'
                ))

fig.add_trace(go.Bar(x=Atención_Hospital['Ubicación del caso'],
                y=Atención_Hospital['Numero'],
                name='Hospital',
                marker_color='rgb(132, 181, 159)'
                ))

fig.add_trace(go.Bar(x=Atención_Hospital_UCI['Ubicación del caso'],
                y=Atención_Hospital_UCI['Numero'],
                name='Hospital UCI',
                marker_color='rgb(163, 201, 168)'
                ))

fig.add_trace(go.Bar(x=Atención_Recuperado['Ubicación del caso'],
...
Fecha.sort_values('Fecha identificación', ascending=False)
...
muer1 = df[df['Recuperado'] == 'Fallecido']

muer3 = muer1['Numero'].count()


Rec1 = df[df['Recuperado'] == 'Recuperado']
